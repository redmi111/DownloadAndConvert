...
public class DownloadMission extends Mission {
    ...
    public DownloadMission(String[] urls, StoredFileHelper storage, char kind, Postprocessing psInstance) {
        ...
        if (BuildConfig.DEBUG && psInstance == null && urls.length > 1) {
     ...
     * Open connection
     *
     * @param threadId   id of the calling thread, used only for BuildConfig.DEBUG
     ...
     * run a new thread
     *
     * @param id  id of new thread (used for BuildConfig.DEBUGging only)
